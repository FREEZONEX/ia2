...
describe("a run started elsewhere", () => {
  ...
  it("does not resurrect a run that stopped before the status arrived", async () => {
    ...
    await act(async () => { answer({ ...idle, running: true, running_info: { kind: "scheduled", programs: ["old_program"] } }) })
    expect(runtime.running).toBeNull()
    expect(runtime.isRunning).toBe(false)
  })
})

it.each([true, false])("ignores an older started response when a newer run is described (stop between runs: %s)", async (stopBetween) => {
  render(<RuntimeProvider><Probe /></RuntimeProvider>)
  await waitFor(() => expect(api.fetchRuntimeStatus).toHaveBeenCalled())
  const answers: Array<(s: RuntimeStatus) => void> = []
  vi.mocked(api.fetchRuntimeStatus).mockImplementation(() => new Promise((resolve) => answers.push(resolve)))
  await deliver({ type: "started" })
  if (stopBetween) await deliver({ type: "stopped" })
  await deliver({ type: "started" })
  await act(async () => answers[1]({ ...idle, running: true, running_info: { kind: "isolated", program: "new_program", file_path: "new_program" } }))
  expect(runtime.running).toEqual({ kind: "isolated", program: "new_program", filePath: "new_program" })
  await act(async () => answers[0]({ ...idle, running: true, running_info: { kind: "isolated", program: "old_program", file_path: "old_program" } }))
  expect(runtime.running).toEqual({ kind: "isolated", program: "new_program", filePath: "new_program" })
  expect(runtime.isRunning).toBe(true)
})
