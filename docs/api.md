...
| `POST` | `/api/desktop/shutdown` | Exit the desktop-owned backend only when no PLC program is running or starting. Returns `202 { ok: true }`. | Enabled only by the desktop's private `IA2_DESKTOP_TOKEN` environment variable; requires `X-IA2-Desktop-Token`. Ordinary server: 404; wrong/missing token: 403; running PLC: 409. Accepted exit atomically rejects subsequent Run requests and drains HTTP connections for up to three seconds. This does not stop a program. |
...
| `POST` | `/api/stop` | Stop the running program cooperatively and wait for the scan thread's failsafe pass and device teardown. Run, Stop, project close and desktop exit serialize their runtime transitions. |
