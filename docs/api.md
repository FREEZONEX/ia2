...
| `POST` | `/api/edges/{name}/deploy` | Tar project + runtime binary + web assets (when this server has a `--static-dir`; they land at `current/web` for the edge's HMI panel), ssh to edge, atomic symlink swap, restart unit, then read the edge runtime's `/status` until the deployed program is known to run (it scanned, and a read a second later shows no fault) or not (a fault, a latched watchdog, or no scan within 30 s). Returns `DeployReport { ok, version, log, warning, health }`. `health` is `DeployHealth { state: running \| faulted \| not_running \| not_checked, detail, unhealthy_devices }`, `null` when the deploy failed before the restart. `ok` is `false` when the remote script failed, and also when the new version is installed and current but its program does not run — nothing is rolled back automatically; the log names the previous version. `not_checked` (no restart happened, or install_dir drift) keeps `ok: true`. A device still down at the check is a `warning`, not a failure.Before any of that, the project goes through the edge runtime's own start path (open with `[governance]` validation, `tasks.toml`, the multi-PROGRAM `VAR_GLOBAL` rule, compile): a project it refuses is `ok: false` with `version: ""`, `health: null` and the reason in `log`, and nothing on the edge changes.An edge with `auto_rollback = true` then rolls back by itself when the deployed program does not run: `current` goes back to the previous version, which is restarted and checked the same way, and `rollback` is `DeployRollback { to, detail, health }` (`to` = the version current again, `null` when nothing was switched). `ok` stays `false`. `rollback` is `null` when the edge does not set it or the program runs. |
